...
class SignInController: UIViewController
{
    ...
    @objc func createnow()
    {
        if let profilename = usernameprofilename.text , let emailaddress = usernamatextfield.text , let password = passwordTextfield.text
        {
            AuthentificationSignIn.system.creatUser(username: profilename, email: emailaddress, password: password) { (Result, error) in
                if let error = error
                {
                    self.alertmessage(title: "User Creation Error", message:"There was an error while creating the User \(error.localizedDescription)", buttonname: "OK") { (method) in
                        print("Message sent ")
                    }
                }else
                {
                   ...
                   {
                    let user = ["email" : emailaddress, "username" : profilename] as [String:Any]
                    Database.database().reference().child("users").child(uid).updateChildValues(user) { (Error, Reference) in
                        if let error  = error
                        {
